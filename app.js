...
const subscriptionsCharged = () => {
  ...
  return data.subscriptions.filter(sub => sub.paidFor === subscriptionPeriod() || currentSubscriptionDue(sub) <= today).reduce((sum, sub) => sum + Number(sub.amount || 0), 0);
};
const available = () => income() - allocated() - transactionsTotal() - goalsTotal() - subscriptionsCharged();
...
function payday() {
  const days = daysUntil(), due = days <= 4, undoOpen = paycheckResetIsOpen(), recent = currentPaycheck(), remaining = Math.ceil(resetWindowRemaining() / 60000);
  if (undoOpen) setTimeout(() => { if (active === 'payday') render(); }, resetWindowRemaining() + 100);
  $('#payday').innerHTML = undoOpen ? `<div class="payday-box"><p>PAYCHECK LOGGED</p><h2>${money(recent.amount)}</h2><p>Need to fix it? Reset is available for ${remaining} more minute${remaining === 1 ? '' : 's'}.</p><button class="secondary" id="resetRecentPaycheck">Reset this paycheck</button></div>` : `<div class="payday-box"><p>${days === 0 ? 'IT\'S PAYDAY' : due ? 'PAYDAY EARLY ACCESS' : 'NEXT PAYDAY'}</p><h2>${due ? 'Ready to log it' : new Date(data.settings.nextPayday + 'T00:00').toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}</h2><p>${due ? (days === 0 ? 'Enter your paycheck and your plan will update immediately.' : `You can log your paycheck up to 4 days early. Payday is in ${days} day${days === 1 ? '' : 's'}.`) : `${days} day${days === 1 ? '' : 's'} to go`}</p>${due ? '<button class="primary" id="openPayday">Log paycheck</button>' : '<p class="muted">Paycheck entry unlocks 4 days before payday.</p>'}</div><div class="section-title"><h2>How it works</h2></div><div class="insight">Your paycheck is divided using your Settings. After you log it, you have 30 minutes to reset only that paycheck if you made a mistake.</div>`;
}

function goals() {
  $('#goals').innerHTML = `<div class="section-title"><h2>Your goals</h2></div>${data.goals.map((goal, index) => { const progress = Math.min(100, goal.saved / goal.target * 100); return `<article class="goal-card"><div class="row"><h3>${escape(goal.name)}</h3><button class="link-btn" data-deposit="${index}">Add money</button></div><p>${money(goal.saved)} of ${money(goal.target)}${goal.date ? ` Â· target ${goal.date}` : ''}</p><div class="progress"><i style="width:${progress}%"></i></div></article>`; }).join('') || '<div class="empty">No goals yet.<br>Create one and move money toward something important.</div>'}<button class="primary" id="addGoal">Create a goal</button>`;
}
